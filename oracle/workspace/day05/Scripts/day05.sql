...
/*PLAYER 테이블에서 TEAM_ID가 'KO1'인 선수 이름을 내 이름으로 바꾸기*/
...
/*표준 문법은 아래와 같으나 아무거나 사용하자.*/
SELECT * FROM PLAYER WHERE NOT "POSITION" IS  NULL;


/*AS(ALIAS): 별칭*/
/*SELET 절과 FROM절에서 사용할 수 있다.*/

/*선수 이름과 생일 조회*/
/*AS 생략도 가능 별칭을 띄어서 쓰고 싶으면 ""로 감싸준다*/
SELECT PLAYER_NAME AS "선수 이름" , BIRTH_DATE AS 선수생일 FROM PLAYER;
SELECT PLAYER_NAME 선수이름 , BIRTH_DATE 선수생일 FROM PLAYER;

/*PLAYER 테이블에서 BACK_NO "등 번호"로, NICKNAME을 "선수 별명"으로 변경하여 검색*/
...
SELECT PLAYER_NAME  || '선수의 POSITION은 ' || POSITION || '이다.' 포지션소개
FROM PLAYER;
 ...
 * 
 * 묶기 전에는 WHERE 절, 묶고 나서는 HAVING절을 사용하여 적절히 문제를 해결할 줄 알아야 한다.
 * 
 * */
...
/*강사님 답안 : PLAYER 테이블에서 평균 키보다 작은 선수 검색, 조회 결과가 맞는 지 검사까지 한 번에 하기
